...
def chart():
    data = sorted(
        tbDemands.objects.values("house_location_name").annotate(
...
def demand_add(request):
    ...
    if user_email:
        ...
        if request.method == "POST":
            ...
            print(
                f"""request.session.get('user_email') : {request.session.get('user_email')}"""
            )
            post_title = request.POST.get("post_title")
            house_type_code = int(request.POST.get("house_type_code"))
            print(
                f"""request.POST.get('house_type_code') : {request.POST.get('house_type_code')}"""
            )
