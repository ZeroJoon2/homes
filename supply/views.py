...
def chart():
    data = sorted(
        tbSupply.objects.values("house_location_name").annotate(
            count=Count("house_location_name")
        ),
...
def supply_view(request):
    ...
    if user_email is not None:
        ...
        if request.method == "POST":
            ...
            post_id = new_row.id
            new_row.save()
            context = {"user": user, "post_id": post_id}
            print(new_row.id)
            print("DB에 저장합니다이")
...
def my_listings_only_view(request):
    ...
    if user_email == user.email:
        ...
        my_listings = tbSupply.objects.filter(email=user_email).values(
            "id", "post_title"
        )

        context = {"my_listings": my_listings, "user": user}
