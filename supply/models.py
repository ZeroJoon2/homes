...
class tbSupply(models.Model):
    ...
    house_transaction_code = models.IntegerField(
        choices=house_transaction_type, default=1
    )
...
class PropertyListing(models.Model):
    list_id = models.ForeignKey(
        tbSupply, on_delete=models.CASCADE, db_column="supply_id"
    )
    # 기본 키가 아닌 특정 필드를 참조하려면 to_field 사용
    email = models.ForeignKey(
        tbUsers, on_delete=models.CASCADE, db_column="email", to_field="email"
    )
